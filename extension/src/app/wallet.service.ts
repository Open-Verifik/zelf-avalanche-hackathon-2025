...
export class WalletService {
    ...
    async initZelfKeySession(): Promise<any> {
        ...
        if (response?.data?.token) {
            ...
            // Store JWT token in extension storage for background script access
            await this._chromeService.setItem("zelfKeyJWT", response.data.token);
            await this._chromeService.setItem("zelfKeyJWTExpiry", this._zelfKeyJWTExpiry);
        }
     ...
     */
    async getZelfKeyJWT(): Promise<string | null> {
        ...
        try {
            const storedJWT = await this._chromeService.getItem("zelfKeyJWT");
            const storedExpiry = await this._chromeService.getItem("zelfKeyJWTExpiry");

            if (storedJWT && storedExpiry && Date.now() < storedExpiry) {
                // Load into memory for faster access
                this._zelfKeyJWT = storedJWT;
                this._zelfKeyJWTExpiry = storedExpiry;
                return this._zelfKeyJWT;
            }
        } catch (error) {
     ...
     */
    async clearZelfKeyJWT(): Promise<void> {
        this._zelfKeyJWT = null;
        this._zelfKeyJWTExpiry = null;

        // Also clear from extension storage
        try {
            await this._chromeService.setItem("zelfKeyJWT", "");
            await this._chromeService.setItem("zelfKeyJWTExpiry", 0);
        } catch (error) {
    ...
    async listStoredPasswords(): Promise<any> {
        ...
        return this._httpWrapper.sendRequest(
            ...
            {
                headers: {
                    Authorization: `Bearer ${await this.getZelfKeyJWT()}`,
                },
            }
        );
    }
